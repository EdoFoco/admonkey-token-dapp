...
contract LotteryPot is Ownable {
    ...
    uint256 constant MIN_DURATION = 300;

    // Basic info
    string public potName;
    uint256 public closedDateTime;
    ...
    mapping(address => uint256) private participantStakes;
    ...
    event WinnerWithdraw(address indexed winner, uint256 indexed totalStake);

    event LotteryPotEnabled(bool indexed enabled);
        ...
        atState(PotState.open)
    {
        ...
        participantStakes[participant] = participantStakes[participant].add(
            msg.value
        );
    ...
    function nextState() internal {
        potState = PotState(uint256(potState) + 1);
    }
    ...
    function determineWinnerInternal() internal view returns (address) {
        ...
        if (potType == PotType.equalShare) {
            uint256 index = getRandom(participants.length);
            return participants[index];
        }

        // Dealing with weightedShare
        uint256 remaining = getRandom(totalStake);
        uint256 index = 0;
    ...
    function getRandom(uint256 len) internal view returns (uint256) {
